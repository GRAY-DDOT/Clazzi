                ...
                trailingIcon = {
                    IconButton(onClick = {
                        ...
                        Icon(
                            ...
                            contentDescription = if (passwordVisible.value) "비밀번호 숨기기" else "비밀번호 표시하기"
                        )
                    }
                },
                keyboardOptions = KeyboardOptions(
                ...
                onClick = {
                    if (isLogin) {
                        ...
                        auth.signInWithEmailAndPassword(email, password)
                            .addOnCompleteListener { task ->
                                if (task.isSuccessful) {
                                    Log.d("AuthScreen", "로그인 성공")
                                    navController.navigate("voteList")
                                } else {
                                    Log.w("AuthScreen", "로그인 실패")
                                }
                            }
                    } else {
                        // 회원가입 로직
                        auth.createUserWithEmailAndPassword(email, password)
                            .addOnCompleteListener { task ->
                                if (task.isSuccessful) {
                                    Log.d("AuthScreen", "회원가입 성공")
                                    navController.navigate("voteList")
                                } else {
                                    Log.w("AuthScreen", "회원가입 실패")
                                }
                            }
                    }
                },
